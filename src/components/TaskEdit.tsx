...
export const TaskEdit: VFC = () => {
  ...
  const { createTaskMutation, updateTaskMutaition } = useMutateTask()
  const submitHandler = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (editedTask.id === 0) createTaskMutation.mutate(editedTask)
    else {
      updateTaskMutaition.mutate(editedTask)
    }
  }
  ...
  if (updateTaskMutaition.isLoading) {
  ...
  return (
    <div>
      ...
      <select
        className="mb-3 px-2 border border-gray-300"
        value={editedTask.id}
        onChange={(e) =>
          dispatch(
